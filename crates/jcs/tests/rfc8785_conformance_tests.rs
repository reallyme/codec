...
//! serialization (§3.2.2.3), within ReallyMe's stricter interoperable-integer
//! policy.
...
fn number_serialization_matches_ecmascript() {
    ...
    // The tiny-magnitude cases are where plain `ryu`/serde diverge from ES6
    // while still remaining inside ReallyMe's interoperable-integer policy.
    let cases: &[(f64, &str)] = &[
        (0.0, "0"),
        (-0.0, "0"),
        (1.0, "1"),
...
fn non_interoperable_integers_are_rejected() {
    ...
    assert_eq!(
        canonicalize_json_text("1e19"),
        Err(JcsError::IntegerOutsideInteroperableRange)
    );
    assert_eq!(
        canonicalize_json_text("9007199254740992.0"),
        Err(JcsError::IntegerOutsideInteroperableRange)
    );
}
