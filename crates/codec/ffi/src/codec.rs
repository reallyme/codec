...
) -> Result<CodecWireError, CodecStatus> {
    CodecWireError::try_new(branch, reason).map_err(|_| CODEC_INTERNAL_ERROR)
}

fn codec_error_proto_bytes(error: CodecWireError) -> ProtoOutput {
    let output = Zeroizing::new(codec_error_bytes(error));
    ProtoOutput {
        bytes: output,
...
fn pem_boundary_error(error: PemError) -> Result<CodecWireError, CodecStatus> {
    match error {
...
fn multikey_boundary_error(error: MultikeyError) -> Result<CodecWireError, CodecStatus> {
    ...
    match operation {
        CODEC_MULTICODEC_PREFIX_FOR_NAME => {
            let Ok(text) = core::str::from_utf8(first) else {
                return Ok(codec_error_proto_bytes(codec_wire_error(
