...
fn cbor_to_tagged(value: CborValue) -> TaggedCborValue {
    match value {
        ...
        CborValue::Int(value) => TaggedCborValue::Int(TaggedInt(value)),
...
pub fn dag_cbor_encode(value_json: &str) -> Result<Uint8Array, JsValue> {
...
pub fn is_valid_cid_string_wasm(cid: &str) -> bool {
    is_valid_cid_string(cid)
}

#[wasm_bindgen(js_name = tryParseCid)]
/// Return the canonical CID string, or `undefined` when parsing fails.
pub fn try_parse_cid_wasm(cid: &str) -> JsValue {
    match codec_cbor::try_parse_cid(cid) {
