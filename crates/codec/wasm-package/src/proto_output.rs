...
use codec_proto::generated::proto::reallyme::codec::v1::{
    ...
    CodecMultikeyParseResult, CodecPemDecodeResult, CodecTag as ProtoCodecTag,
};
use codec_proto::{codec_error_bytes, CodecWireError, CodecWireErrorBranch};
...
) -> Result<CodecWireError, JsValue> {
    CodecWireError::try_new(branch, reason).map_err(|_| provider_failure())
}

fn encode_error_proto(error: CodecWireError) -> ProtoOutput {
    ProtoOutput {
        status: ProtoStatus::CodecError,
        bytes: Zeroizing::new(codec_error_bytes(error)),
    }
}
...
fn pem_boundary_error(error: PemError) -> Result<CodecWireError, JsValue> {
    match error {
        PemError::InputTooLarge => codec_wire_error(
            CodecWireErrorBranch::BaseEncoding,
            CodecErrorReason::CODEC_ERROR_REASON_BASE_INPUT_TOO_LARGE,
        ),
        PemError::DerTooLarge => codec_wire_error(
        ...
        | PemError::InvalidOptions => codec_wire_error(
            CodecWireErrorBranch::Pem,
            CodecErrorReason::CODEC_ERROR_REASON_PEM_INVALID_BOUNDARY,
        ),
        PemError::LabelMismatch => codec_wire_error(
            CodecWireErrorBranch::Pem,
            CodecErrorReason::CODEC_ERROR_REASON_PEM_LABEL_MISMATCH,
        ),
        PemError::UnsupportedLabel => codec_wire_error(
            CodecWireErrorBranch::Pem,
            CodecErrorReason::CODEC_ERROR_REASON_PEM_UNSUPPORTED_LABEL,
        ),
        PemError::InvalidBase64 | PemError::InvalidBody => codec_wire_error(
            CodecWireErrorBranch::Pem,
...
fn multikey_boundary_error(error: MultikeyError) -> Result<CodecWireError, JsValue> {
    ...
    codec_wire_error(CodecWireErrorBranch::Multiformat, reason)
}
...
fn multicodec_prefix_for_name_proto_output(codec_name: &str) -> Result<ProtoOutput, JsValue> {
    let Some((name, spec)) = find_codec_spec(codec_name) else {
        return Ok(encode_error_proto(codec_wire_error(
...
fn dag_cbor_verify_cid_proto_output(cid: &str, bytes: &Uint8Array) -> Result<ProtoOutput, JsValue> {
    let bytes = Zeroizing::new(bytes.to_vec());
...
fn pem_decode_proto_output(input: &str, options_json: &str) -> Result<ProtoOutput, JsValue> {
    ...
    let decoded = match decode_pem(input, policy) {
        Ok(decoded) => decoded,
        Err(error) => return Ok(encode_error_proto(pem_boundary_error(error)?)),
    };
